...
beta_ci95_lo, beta_ci95_hi = scipy.stats.t.interval(0.95, df=n-p, loc=beta_hat, scale=np.sqrt(beta_var*xtxinv_diag)) # 95% confidence intervals
...
beta_var_1 = (1/(n_1-p_1)) * (ups_1 - X_1@beta_hat_1).T@(ups_1 - X_1@beta_hat_1)

xtxinv_1 = scipy.linalg.inv(X_1.T@X_1)
xtxinv_diag_1 = np.diag(xtxinv_1)

beta_ci95_lo_1, beta_ci95_hi_1 = scipy.stats.t.interval(0.95, df=n_1-p_1, loc=beta_hat_1, scale=np.sqrt(beta_var_1*xtxinv_diag_1)) # 95% confidence intervals
...
beta_var_2 = (1/(n_2-p_2)) * (ups_2 - X_2@beta_hat_2).T@(ups_2 - X_2@beta_hat_2)

xtxinv_2 = scipy.linalg.inv(X_2.T@X_2)
xtxinv_diag_2 = np.diag(xtxinv_2)

beta_ci95_lo_2, beta_ci95_hi_2 = scipy.stats.t.interval(0.95, df=n_2-p_2, loc=beta_hat_2, scale=np.sqrt(beta_var_2*xtxinv_diag_2)) # 95% confidence intervals
